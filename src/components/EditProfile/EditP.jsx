...
const EditP = () => {
   ...
   const handleSummit = async (e) =>{
    ...
    try{
        setLoad(true)
        const res = await axios.put(`${config.apiuserupdata}/${user._id}`,data,configsheaders) 
         ...
         Swal.fire({
            ...
            if (result.isConfirmed) {
               Swal.fire(
                  ...
                  await axios.delete(`${config.apiuserDelete}/${user?._id}`).then(
                                 ...
                                 <div className="me-s-c">
                                   <div className="n-s-c">Gender</div>
                                       <input id="m"  
                                         ...
                                         />
                                       <label className='button' htmlFor="m">ชาย</label>
                                          <input id="w"  
                                             name='gender'
                                             className='c-se'
                                             type='radio'
                                             ...
                                             onChange={(e) => setGender(e.target.value)}
                                             />
                                       <label htmlFor="w" className='button'>หญิง</label>
                                          <input id="no" 
                                             name='gender' 
                                             className='c-se' 
                                             type='radio'
                                             ...
                                             onChange={(e) => setGender(e.target.value)}
                                             />
                                       <label htmlFor="no" className='button'>ไม่ระบุ</label>
                                 </div>
                                 <div className="me-b-setting">
                                     <button className='btn-delete' onClick={()=> handleDeleteUser(user)}>ลบบัญชีผู้ใช้งาน</button>
                                     <button onClick={handleSummit} className='btn-save'>save</button>
                                 </div>
                              </div>
                              <div className="im-pro-dit let">
                                  <label className='ora-ty w-h font-s-c' htmlFor="imgeProfile"><h2>Edit</h2></label>
                                     <div id='imgset' className="box-im-pf">
                                     <img src={image ? image : user?.avatar?.avatar_url || Novatar} alt="" />
                                    </div>
                                     <input  id='imgeProfile' 
